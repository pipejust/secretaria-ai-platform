...
- El bot lee **un solo buzón IMAP** con **una sola dirección para todas las empresas** (`bot@acten.app`, `MAILBOX_JSON` en Coolify, app del bot), como el asistente de Fireflies. El bot sabe de qué empresa es una invitación por el **remitente**: Acten copia a la política del bot los correos de los usuarios activos de cada empresa que acepte el bot propio (`services/mail_policy_sync.py`: al guardar la política, al cambiar el origen de reuniones y cada hora), más los «remitentes adicionales» que escriba el administrador. Un remitente autorizado en dos empresas se rechaza para ambas (`email_ambiguous_tenant`), visible en «Ver recepción de invitaciones».
